...
def handle_clients(conn,address):
    ...
    welcome = f"Welcome {name}. Type #quit if you want to leave"
    conn.send(bytes(welcome,'utf8'))
    msg = f"{name} has just join the room"
...
def broadcast(msg, prefix=''):
    for i in clients:
        i.send(bytes(prefix,"utf8")+msg)
        

if __name__ == '__main__':
    
    sock.listen(5)
    print('The server is listening for client requests')
